...
if (!/^(darwin|win32|linux)$/i.test(process.platform)) {
  process.stdout.write(`Operating system "${process.platform}" is not supported! Please open an issue at "https://github.com/nlstone/Markdown-PlusPlus".\n`)
  process.exit(1)
}
...
// Make MarkDown++ a single instance application.
if (!process.mas && process.env.NODE_ENV !== 'development') {
  const gotSingleInstanceLock = app.requestSingleInstanceLock()
  if (!gotSingleInstanceLock) {
    process.stdout.write('Other MarkDown++ instance detected: exiting...\n')
    app.exit()
  }
}

// MarkDown++ environment is configured successfully. You can now access paths, use the logger etc.
