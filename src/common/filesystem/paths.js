...
const isIgnoredWatcherSegment = segment => {
  if (!segment || segment === '.' || segment === '..') {
    return false
  }

  if (segment === 'node_modules' || /\.asar$/i.test(segment)) {
    return true
  }

  return segment.startsWith('.') && !WATCHER_ALLOWED_HIDDEN_SEGMENTS.has(segment)
}
...
/**
 * Returns true if the filesystem watcher should ignore the path.
 *
 * Hidden directories are ignored by default, except explicitly allowed
 * workspace-managed paths such as ".zread".
 *
 * @param {string} pathname Path to test.
 * @param {{ isDirectory: Function }} [fileInfo] Optional fs.Stats-like object.
 */
export const shouldIgnoreWatcherPath = (pathname, fileInfo) => {
  if (!pathname || typeof pathname !== 'string') {
    return true
  }

  const segments = splitPathSegments(pathname)
  if (segments.some(isIgnoredWatcherSegment)) {
    return true
  }

  if (!fileInfo || typeof fileInfo.isDirectory !== 'function') {
    return false
  }
